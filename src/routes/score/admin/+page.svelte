<script lang="ts">
	...
	let draftScores = $state({ ...data.scores.scores });
	...
	onMount(() => {
		const refresh = async () => {
			try {
				...
				if (response.ok) applyScoreboard(await response.json());
			}
	...
	async function request(body: Record<string, unknown>) {
		...
		try {
			...
			applyScoreboard(await response.json()); message = '';
		} catch { message = '저장하지 못했습니다. 다시 시도해 주세요.'; }
		finally { isSaving = false; }
	}
	function applyScoreboard(next: typeof scoreboard) {
		scoreboard = next;
		draftScores = { ...next.scores };
	}
	...
	function saveScore(teamId: TeamId) {
		const score = Number(draftScores[teamId]);
		if (!Number.isSafeInteger(score)) {
			message = '점수는 정수만 입력할 수 있습니다.';
			draftScores = { ...scoreboard.scores };
			return;
		}
		request({ action: 'set', teamId, score });
	}
	...
	<div class="admin-grid" aria-label="점수 관리">
		{#each TEAM_IDS as team}
			<article class="admin-card {team}">
				...
				<input class="score-input" type="number" aria-label={`${scoreboard.names[team]} 점수`} bind:value={draftScores[team]} onblur={() => saveScore(team)} onkeydown={(event) => event.key === 'Enter' && (event.currentTarget as HTMLInputElement).blur()} />
...
<style>
	.admin-page { position: relative; min-height: 100vh; padding: clamp(26px, 5vw, 72px); display: grid; place-items: center; background: #fff; }.admin-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: clamp(12px, 2vw, 28px); width: min(1420px, 100%); }.admin-card { position: relative; min-height: clamp(330px, 43vw, 520px); padding: 26px; border-radius: 30px; color: #fff; box-shadow: 0 18px 42px rgb(24 31 45 / 14%); }.coral { background: #f5a293; }.sky { background: #56ccf2; }.violet { background: #9f87dd; }.green { background: #6fcf97; }.admin-card > input:not(.score-input) { position: absolute; top: 26px; left: 24px; width: calc(100% - 48px); border: 0; border-bottom: 2px solid rgb(255 255 255 / 52%); border-radius: 0; background: transparent; color: #fff; font: 800 clamp(17px, 1.7vw, 25px) var(--font-sans); letter-spacing: -.04em; text-align: center; outline: none; }.admin-card > input:not(.score-input):focus { border-bottom-color: #fff; }.score-input { position: absolute; top: 56%; left: 50%; width: min(100%, 290px); border: 0; background: transparent; color: #fff; font: 800 clamp(88px, 12vw, 184px) / 1 var(--font-sans); letter-spacing: -.1em; text-align: center; font-variant-numeric: tabular-nums; outline: none; transform: translate(calc(-50% - 10px), -50%); }.score-input::-webkit-inner-spin-button, .score-input::-webkit-outer-spin-button { appearance: none; margin: 0; }.arrow { position: absolute; left: 50%; width: 58px; height: 58px; border: 0; border-radius: 50%; background: rgb(255 255 255 / 22%); color: #fff; cursor: pointer; font: 700 36px / 1 var(--font-sans); transform: translateX(-50%); }.arrow:hover { background: rgb(255 255 255 / 36%); }.arrow:not(:disabled):active { transform: translateX(-50%) scale(.97); }.arrow:disabled, .reset:disabled { cursor: wait; opacity: .55; }.up { top: clamp(72px, 8vw, 106px); }.down { bottom: 24px; }.reset { position: absolute; right: clamp(26px, 5vw, 72px); bottom: 20px; border: 0; border-radius: 999px; padding: 10px 15px; background: #f2f3f5; color: #566170; cursor: pointer; font: 700 13px var(--font-sans); }.message { position: absolute; bottom: 20px; left: clamp(26px, 5vw, 72px); margin: 0; color: #b24b4b; font-size: 13px; }@media (max-width: 760px) { .admin-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }.admin-card { min-height: 330px; border-radius: 22px; }.admin-card > input:not(.score-input) { top: 20px; left: 18px; width: calc(100% - 36px); }.arrow { width: 46px; height: 46px; font-size: 29px; }.up { top: 62px; }.down { bottom: 18px; }.score-input { top: 58%; } }@media (max-width: 420px) { .admin-grid { gap: 10px; }.admin-card { min-height: 330px; padding: 18px; }.admin-card > input:not(.score-input) { font-size: 15px; } }
</style>
